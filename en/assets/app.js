...
const getPageHref = file => {
  const isInPagesDirectory = window.location.pathname.includes("/pages/");
  return isInPagesDirectory ? `./${file}` : `./pages/${file}`;
};
...
const applySiteConfig = () => {
  ...
  document.querySelectorAll(".brand").forEach(brand => {
    ...
    brand.setAttribute("aria-label", `${siteConfig.siteName} home`);
    const label = brand.querySelector("[data-site-name]") || brand.querySelector("span:not(.brand-mark)");
    if (label) {
  ...
  document.querySelectorAll(".site-nav").forEach(nav => {
    ...
    nav.querySelectorAll("a").forEach(link => {
      const item = siteConfig.nav.find(navItem => link.getAttribute("href")?.endsWith(navItem.file));
      if (!item) {
        return;
      }

      link.href = getPageHref(item.file);
  ...
  setText("[data-home-language]", home.language);
  ...
  setHtml("[data-profile-title]", profile.title);
  setText("[data-profile-lead]", profile.lead);
  setText("[data-profile-portrait]", profile.portraitPlaceholder);
  setText("[data-profile-name]", profile.identity?.name);
  setText("[data-profile-role]", profile.identity?.role);
  setText("[data-profile-location]", profile.identity?.location);

  const profileAbout = document.querySelector("[data-profile-about]");
  if (profileAbout && Array.isArray(profile.about)) {
    profileAbout.innerHTML = profile.about.map(paragraph => `<p>${escapeHtml(paragraph)}</p>`).join("");
  }

  const profileEducation = document.querySelector("[data-profile-education]");
  if (profileEducation && Array.isArray(profile.education)) {
    profileEducation.innerHTML = profile.education.map(item => `
      <div>
  ...
  setText("[data-profile-projects]", profile.projects);
  setText("[data-profile-experience]", profile.experience);
  setText("[data-profile-awards]", profile.awards);
  setText("[data-profile-contact-text]", profile.contactText);
  setText("[data-profile-contact-link]", profile.contactLinkLabel);

  const profileSkills = document.querySelector("[data-profile-skills]");
  if (profileSkills && Array.isArray(profile.skills)) {
    profileSkills.innerHTML = profile.skills.map(skill => `<li>${escapeHtml(skill)}</li>`).join("");
  }

  document.querySelectorAll("[data-profile-image-placeholder]").forEach(item => {
    const index = Number(item.dataset.profileImagePlaceholder || 0);
    if (profile.imagePlaceholders?.[index]) {
      item.textContent = profile.imagePlaceholders[index];
    }
  });
  ...
  document.querySelectorAll("[data-gallery-item]").forEach((item, index) => {
    if (siteConfig.gallery?.items?.[index]) {
      item.textContent = siteConfig.gallery.items[index];
    }
  });
  ...
  if (contactBg && contact.background) {
    contactBg.src = contact.background;
  }
...
const journalStorageKey = "personalHomePageJournalEntries";
  ...
  if (mediaRoot && slidesRoot && entries.length) {
    mediaRoot.innerHTML = entries.map((entry, index) => `
      <div class="journal-media-set${index === 0 ? " is-active" : ""}" data-journal-media="${index}">
        <figure class="journal-image-frame journal-media-primary"><img src="${escapeHtml(entry.images[0])}" alt=""></figure>
        <figure class="journal-image-frame journal-media-secondary"><img src="${escapeHtml(entry.images[1])}" alt=""></figure>
      </div>
    `).join("");
    ...
    slidesRoot.innerHTML = entries.map((entry, index) => `
      <article class="journal-slide${index === 0 ? " is-active" : ""}" data-journal-slide>
        ...
        <a class="journal-title-link" href="/en/journal-entry/?entry=${encodeURIComponent(entry.slug)}">
          ...
          <div class="gallery-loop-track">
            ${repeatedItems.map((item, itemIndex) => `
              <figure class="gallery-loop-card">
                <img src="${escapeHtml(item.src)}" alt="${escapeHtml(item.alt || `Gallery image ${itemIndex + 1}`)}" loading="lazy">
              </figure>
            `).join("")}
          </div>
        </div>
...
(() => {
  ...
  if (!entry) {
    detailRoot.innerHTML = `<p class="journal-detail-fallback">No journal entry found.</p>`;
    return;
  }
  ...
  detailRoot.innerHTML = `
    <div class="journal-detail-actions">
      <a class="journal-detail-back" href="/en/journal/">Back</a>
      <button class="journal-detail-button" type="button" data-journal-edit>Edit</button>
      <button class="journal-detail-button journal-detail-save" type="button" data-journal-save hidden>Save</button>
    </div>
    <article class="journal-detail-article">
      ...
      <section class="journal-detail-media" aria-label="${escapeHtml(entry.title)} images">
        <figure>
          <img src="${escapeHtml(entry.images[0])}" alt="">
          <label class="journal-image-editor">Image 1 <input value="${escapeHtml(entry.images[0])}" data-edit-image="0" disabled></label>
        </figure>
        <figure>
          <img src="${escapeHtml(entry.images[1])}" alt="">
          <label class="journal-image-editor">Image 2 <input value="${escapeHtml(entry.images[1])}" data-edit-image="1" disabled></label>
        </figure>
      </section>
  ...
  saveButton?.addEventListener("click", () => {
    ...
    document.title = `${updatedEntry.title} | Journal`;
    setEditable(false);
  });
})();
...
if (isHomePage) {
  ...
  const updateTitleSplit = () => {
    if (!visualPanel || !homeTitle) {
      return;
    }
